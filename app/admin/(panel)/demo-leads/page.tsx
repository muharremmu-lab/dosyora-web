...
export default async function DemoLeadsPage({ searchParams }: DemoLeadsPageProps) {
  ...
  const result = listDemoLeads({ page, limit: 20, search, accountStatus, source })

  return (
    <AdminShell title="Demo Hesapları" description="Otomatik oluşturulan demo hesaplarını yönetin">
      <Card className="mb-6">
        <form method="get" className="grid gap-4 md:grid-cols-4" aria-label="Demo hesapları filtre formu">
        ...
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-[var(--ds-color-border)] bg-[var(--ds-color-surface-alt)]">
            <tr>
              ...
              <th className="px-4 py-3 font-semibold text-[var(--ds-color-text)]">Mail</th>
